...
public final class SuperReforgeConfig {
    ...
    public static final class ServerValues {
        ...
        private final ModConfigSpec.ConfigValue<String> automaticCatalyst;
        ...
        private ServerValues(ModConfigSpec.Builder builder) {
            ...
            automaticCatalyst = builder
                    .comment("自动首词条开启时，用哪个 catalyst 定义决定等级和词条概率。")
                    .define("automaticCatalyst", defaults.automaticCatalyst().toString());
    ...
    /** 配置字符串损坏时回退到可运行默认值，并由 NeoForge 配置校验在下次保存时纠正。 */
    private static ResourceLocation automaticCatalyst() {
        ResourceLocation parsed = ResourceLocation.tryParse(SERVER.automaticCatalyst.get());
        return parsed == null ? GlobalSettings.DEFAULTS.automaticCatalyst() : parsed;
    }
}

...
本教程面向 Minecraft `1.21.1`、Java `21`，以及 NeoForge `21.1.244` 或后续仍兼容 Minecraft `1.21.1` 的构建。Super Reforge 是双端必需模组；Curios 9.x 与 KubeJS 2101 是双端可选依赖。不安装它们时，原版武器和数据包重铸仍可使用。
...
1. 在熔核锻台中放入 **数量恰为 1** 的目标物品；整叠物品不能作为一次重铸目标。
2. 放入能匹配某个媒介定义 `ingredient` 的物品。媒介还会限制允许/禁止的物品类型，并提供候选品质、材料数量与经验成本。
3. 查看服务器权威的概率预览：完整列表可用滚轮浏览，也可拖动滚动滑块定位；悬停词条行可查看词条 ID，以及每条效果的 Attribute 资源 ID、固定值或范围、运算方式和槽位。确认目标、媒介和候选词条均有效后开始重铸。
4. 正常情况下会扣除媒介定义的 `count` 个材料，以及其 `experience` 经阶段修正后的经验成本；经验是否启用及按等级还是经验点支付由服务器配置决定。
5. 默认动画为 `20 tick`（约 1 秒）；动画锻锤以锤头向下的竖直姿态完成抬起、接触、回弹和复位。该姿态属于视觉修正，不改变重铸规则。完成后，物品获得抽中的词条。铁砧重命名不会被词条名称写死或丢失。媒介定义默认不允许在仍有其他候选词条时重复抽到当前词条。

若预览达到网络展示安全上限，锻造日志会显示截断警告。这表示发送给客户端的展示内容不是完整集合；服务端仍使用完整候选池进行报价和抽取，警告不会改变实际抽取判定。

创造模式且 `creativePlayersPay = false` 时，材料和经验成本为 0；但仍须放入一个可匹配媒介定义的物品，且该媒介必须能产生有效候选池。自动首词条功能也只处理数量为 1 的物品，并不会经过熔核锻台、消耗材料或经验。

独立 `Super Reforge` 创造模式页签只公开四项内容：熔核锻台、普通重铸石、精炼重铸石和至高重铸石。动画专用锻锤用于方块实体渲染，不作为普通物品在创造页签中展示。
...
| 无法开始重铸且提示定义同步 | 等待客户端完成当前服务端定义的同步；在数据包或脚本 reload 后重新打开熔核锻台并检查同步是否完成。 |
| 熔核锻台正在执行上一轮重铸 | 等待当前锻造动画完成后再开始下一次重铸。 |

...
5. 默认动画为 `20 tick`（约 1 秒）；完成后，物品获得抽中的词条。铁砧重命名不会被词条名称写死或丢失。
...
- 确认最终 JAR 文件名、下载地址、支持语言、许可证展示文案与截图素材。
- 确认已在目标客户端、专用服务器，以及安装和未安装 Curios、KubeJS 的组合环境完成所需实机验收。
- 确认投稿页的游戏版本与依赖展示和实际上传 JAR 中的 `META-INF/neoforge.mods.toml` 一致。

本教程仅依据仓库中的配置、API 和示例编写；上述运行环境组合的实机验收仍应由作者在投稿前确认。

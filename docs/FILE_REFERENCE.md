...
- `examples/kubejs/superreforge_curio_progression.js`：MUTUO 的纯 Curios 八级配置；解析 `SR_CURIO_SPEC` 注册 32 条固定饰品词条与六级媒介精确权重，用零权重/空气类型覆盖内置 24 条词条，并统一把效果限制为 `curios:any`。1–3 级使用现有三种石头，4–6 级使用原版下界合金锭、下界之星和龙息，因此只替换 KubeJS 脚本即可。
